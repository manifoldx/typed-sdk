...
A network provider is an abstracts connection to the protocol, for issuing queries
and sending signed state-changing transactions.

The *JsonRpcProvider* allows you to connect to protocol nodes that you
...
There are several methods to connect to the protocol network provider. If you are not
running your own local protocol node, it is recommended that you use the ``getDefaultProvider()``
method.

:sup:`manifold` . getDefaultProvider( [ network = "localnet" ] ) |nbsp| `=> Provider`
    This creates a FallbackProvider backed by multiple backends.
    
    This is the **recommended** method of connecting to the protocol network if you are
    not running your own protocol node.
...
new :sup:`manifold . providers` . JsonRpcProvider( [ urlOrInfo :sup:`= "http://localhost:26657"` ] [ , network ] )
    Connect to the `JSON-RPC API`_ URL *urlOrInfo* of a protocol node.
