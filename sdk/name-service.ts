...
export class Alias {
   ...
   */
  static signAliasStatusTransaction(
    transaction: AliasStatusTransaction,
    signer: Signer,
    overrides?: any
  ) {
    if (!Signer.isSigner(signer)) {
      errors.throwError(
        "sign alias status transaction require signer",
   ...
   * @param overrides options
   */
  static sendAliasStatusTransaction(
    transaction: AliasStatusTransaction,
    signer: Signer,
    overrides?: any
  ): Promise<TransactionResponse | TransactionReceipt> {
    return this.getAliasStatusTransactionRequest(
      transaction,
      signer,
      overrides
    ).then((tx) => {
  ...
  static getAliasStatusTransactionRequest(
    transaction: AliasStatusTransaction,
    signer: Signer,
    overrides?: any
    ...
    if (!Signer.isSigner(signer)) {
      errors.throwError(
        "send alias status transaction require signer",
        errors.MISSING_ARGUMENT,
        { arg: "signer" }
      );
    }
...
function setAliasStatus(
  ...
  if (!Signer.isSigner(signer)) {
    errors.throwError(
      "set alias status transaction require signer",
      errors.MISSING_ARGUMENT,
      { arg: "signer" }
    );
  }
  ...
  if (!name) {
    errors.throwError(
      "set alias status transaction require name",
      errors.MISSING_ARGUMENT,
      { arg: "name" }
    );
  }
  ...
  switch (status) {
    ...
    default:
      errors.throwError("invalid alias status", errors.UNEXPECTED_ARGUMENT, {
        arg: "status",
        value: status,
      });
  }
  ...
  return resolveProperties({
    ...
    return resolveProperties({ nonce: nonce }).then(({ nonce }) => {
      ...
      return signer
        .signMessage(JSON.stringify(transaction.payload.alias), true)
        .then((signature) => {
