  ...
  return camelize(
    ...
    (key) => {
      switch (key) {
        ...
        case "EndorserList":
          return "endorserList";
        case "EndorserListLimit":
          return "endorserListLimit";
      }
      return key;
    }
  );
...
function checkBlock(data: any): Block {
  ...
  if (
    ...
    !Array.isArray(block.results.transactions)
  ) {
    block.results.transactions = [];
  } else {
...
function checkBlockInfo(data: any): BlockInfo {
  ...
  if (data.block && data.block.header) {
    data.block.header.proposerAddress = computeAddress(
...
function checkTransactionEvent(data: any): TransactionEvent {
  ...
  let event: TransactionEvent = camelize(
    checkFormat(
      {
        hash: checkString,
...
function checkTransactionEvents(data: any): any {
  let groups: TypeAttribute[] = allowNullOrEmpty(arrayOf(checkTypeAttribute))(
    data
  );
  let events: TransactionEvent[] = [];
...
function checkTransactionReceipt(transaction: any): TransactionReceipt {
  let receipt: TransactionReceipt = camelize(
    checkFormat(
      {
        hash: checkHash,
        height: checkNumber,
...
function checkTransactionLog(data: any): TransactionLog {
  ...
  let log: TransactionLog = camelize(
    checkFormat(
      {
...
export class BaseProvider extends Provider {
  ...
  // To help mitigate the eventually conssitent nature of the protocol
  ...
  getTransactionFee(
    ...
    return this.ready.then(() => {
      ...
      let params = {
        unsignedTransaction: base64Encode(
          toUtf8Bytes(JSON.stringify(transaction))
        ),
      };
  ...
  getTokenState(
    ...
    return this.ready.then(() => {
      return resolveProperties({ symbol: symbol, blockTag: blockTag }).then(
        ({ symbol, blockTag }) => {
          let params = {
            symbol: symbol,
  ...
  getTokenAccountState(
    ...
    return this.ready.then(() => {
      return resolveProperties({
        ...
        blockTag: blockTag,
      }).then(({ symbol, addressOrName, blockTag }) => {
  ...
  getTokenAccountBalance(
    ...
    return this.getTokenAccountState(symbol, addressOrName, blockTag).then(
      (state) => {
  ...
  getNFTokenItemState(
    ...
    itemID: string,
    blockTag?: BlockTag | Promise<BlockTag>
  ): Promise<NFTokenItemState> {
    return this.ready.then(() => {
      return resolveProperties({
        symbol: symbol,
        itemID: itemID,
        blockTag: blockTag,
      }).then(({ symbol, itemID, blockTag }) => {
        let params = {
          symbol: symbol,
          itemID: itemID,
          blockTag: checkBlockTag(blockTag),
  ...
  getAliasState(
    ...
    return resolveProperties({ address, blockTag }).then(
      ({ address, blockTag }) => {
        return this.ready.then(() => {
          let params = {
            address,
  ...
  sendTransaction(
    ...
    return this.ready.then(() => {
      return resolveProperties({ signedTransaction: signedTransaction }).then(
        ({ signedTransaction }) => {
          let params = { signedTransaction };
          let async = overrides && overrides.async ? true : false;
          return this.perform(
            async ? "sendTransactionAsync" : "sendTransaction",
            params
          ).then(
            (result) => {
          ...
          return poll(
            () => {
              let promises = [
                ...
                this.perform("getBlockInfo", {
                  blockTag: (blockNumber + 1).toString(),
                }),
              ];
              return Promise.all(promises).then((results) => {
                if (
                  isUndefinedOrNullOrEmpty(results) ||
                  isUndefinedOrNullOrEmpty(results[0]) ||
                  isUndefinedOrNullOrEmpty(results[1])
                ) {
  ...
  getTransactionReceipt(transactionHash: string): Promise<TransactionReceipt> {
    return this.ready.then(() => {
      return resolveProperties({ transactionHash: transactionHash }).then(
        ({ transactionHash }) => {
          let params = { transactionHash: checkHash(transactionHash, true) };
          return poll(
            () => {
              return this.perform("getTransactionReceipt", params).then(
                (result) => {
                  if (result == null) {
  ...
  resolveName(
    ...
    return resolveProperties({ name: name, blockTag: blockTag }).then(
      ({ name, blockTag }) => {
        ...
        return this.ready.then(() => {
          ...
          return this.perform("resolveName", params).then((address) => {
            if (!address) {
              errors.throwError("invalid address", errors.INVALID_ADDRESS, {
                value: name,
              });
            }
            return address;
  ...
  lookupAddress(
    ...
    return resolveProperties({ address: address, blockTag: blockTag }).then(
      ({ address, blockTag }) => {
        ...
        Object.keys(uniqueEventTags).forEach((tag) => {
          ...
          switch (comps[0]) {
            ...
            case "address": {
              ...
              this.getBalance(address, "latest")
                .then((balance) => {
...
function checkBlockTag(blockTag: BlockTag): string {
  ...
  if (isHexString(blockTag)) {
    return bigNumberify(blockTag).toString();
  }

  try {
    if ("string" === typeof blockTag) return parseInt(blockTag).toString();
