...
export class JsonRpcProvider extends BaseProvider {
  ...
  constructor(url?: ConnectionInfo | string, network?: Networkish) {
    ...
    if (
      ...
      0 < this.connection.pollingInterval
    ) {
  ...
  perform(method: string, params: any): Promise<any> {
    switch (method) {
      case "sendTransaction":
        return this.send("encode_and_broadcast_tx_sync", [
      ...
      case "sendTransactionAsync":
        return this.send("encode_and_broadcast_tx_async", [
          params.signedTransaction,
        ]).then((result) => {
          if (0 == result.code) {
            if (result.hash) {
      ...
      case "getTransactionReceipt":
        return this.send("decoded_tx", [
          base64Encode(params.transactionHash),
          null,
        ]).then((result) => {
      ...
      case "getTransactionFee":
        return this.send("query_fee", [params.unsignedTransaction]).then(
          (result) => {
      ...
      case "getKycAddress":
        return this.send("abci_query", [
          "/custom/kyc/get_kyc_address/" + params.address,
          "",
          params.blockTag,
          null,
        ]).then((result) => {
      ...
      case "getTokenState":
        return this.send("abci_query", [
          "/custom/token/token_data/" + params.symbol,
          "",
          params.blockTag,
          null,
        ]).then((result) => {
      ...
      case "getTokenList":
        return this.send("abci_query", [
          "/custom/token/list-token-symbol",
          "",
          params.blockTag,
          null,
        ]).then((result) => {
      ...
      case "getTokenAccountState":
        return this.send("abci_query", [
          "/custom/token/account/" + params.symbol + "/" + params.address,
          "",
          params.blockTag,
          null,
        ]).then((result) => {
      ...
      case "getNFTokenState":
        return this.send("abci_query", [
          "/custom/nonFungible/token_data/" + params.symbol,
          "",
          params.blockTag,
          null,
        ]).then((result) => {
      ...
      case "resolveName":
        return this.send("abci_query", [
          "/custom/nameservice/resolve/" + params.name,
          "",
          params.blockTag,
          null,
        ]).then((result) => {
      ...
      case "lookupAddress":
        return this.send("abci_query", [
          "/custom/nameservice/whois/" + params.address,
          "",
          params.blockTag,
          null,
        ]).then((result) => {
      ...
      case "getAliasState":
        return this.send("abci_query", [
          "/custom/nameservice/pending/" + params.address,
          "",
          params.blockTag,
          null,
        ]).then((result) => {
  ...
  checkResponseLog(
    ...
    return checkResponseLog(
      this,
      method,
      result,
      isUndefinedOrNullOrEmpty(code) ? errors.UNEXPECTED_RESULT : code,
      message,
      params
    );
  }
...
function checkResponseLog(
  self: JsonRpcProvider,
  method: string,
  result: any,
  defaultCode: string,
          ...
          if (result.response && result.response.log) {
            ...
            if (
              ...
              if (
                ...
                result.result.logs[0].info &&
                result.result.logs[0].info.message
              ) {
      ...
      case "sdk":
        switch (info.code) {
              ...
              switch (info.message) {
                case "Endorserlist limit exceeded.": // error code 2113
                  return errors.createError(
                    "token endorserlist limit exceeded",
                    errors.NOT_ALLOWED,
                    { operation: method, info, response: result, params }
                  );
                case "Endorserlist limit cannot less than or equal 0":
                  return errors.createError(
                    "token endorserlist limit is not valid",
          ...
          case 11: // Invalid amount
            return errors.createError("invalid amount", errors.NUMERIC_FAULT, {
              operation: method,
              info,
              response: result,
              params,
      ...
      case "manifold":
        switch (info.code) {
          ...
          case 1001: // KYC address duplicated
            return errors.createError("Duplicated KYC", errors.EXISTS, {
              operation: method,
              info,
              response: result,
              params,
          ...
          case 1002: // Receiver KYC is required
            return errors.createError(
              "Receiver KYC is required",
              errors.RECEIVER_KYC_REQUIRED,
              { operation: method, info, response: result, params }
            );

          case 2001: // Token already exists
            return errors.createError("token exists", errors.EXISTS, {
              operation: method,
              info,
              response: result,
              params,
          ...
          case 2002: // Token does not exists
            return errors.createError("token not found", errors.NOT_FOUND, {
              operation: method,
              info,
              response: result,
              params,
          ...
          case 2004: // Token is frozen
            return errors.createError("token is frozen", errors.NOT_ALLOWED, {
              operation: method,
              info,
              response: result,
              params,
          ...
          case 2006: // Invalid token
            return errors.createError("invalid token", errors.NOT_AVAILABLE, {
              operation: method,
              info,
              response: result,
              params,
          ...
          case 2104: // Transfer token ownership approved
            return errors.createError(
              "token ownership is already approved",
              errors.EXISTS,
              { operation: method, info, response: result, params }
            );
          case 2105:
            return errors.createError(
              "token item id is in used",
              errors.EXISTS,
              { operation: method, info, response: result, params }
            );
          case 2106:
            return errors.createError(
              "token item endorser invalid",
              errors.NOT_ALLOWED,
              { operation: method, info, response: result, params }
            );
          case 2107:
            return errors.createError("token item frozen", errors.NOT_ALLOWED, {
              operation: method,
              info,
              response: result,
              params,
          ...
          case 4001: // Alias in used
            return errors.createError("alias in used", errors.EXISTS, {
              operation: method,
              info,
              response: result,
              params,
          ...
          case 4003: // Alias not allowed to create
            return errors.createError(
              "not allowed to create alias",
              errors.NOT_ALLOWED,
              { operation: method, info, response: result, params }
            );
          case 4004: // Alias not found
            return errors.createError("alias not found", errors.NOT_FOUND, {
              operation: method,
              info,
              response: result,
              params,
  ...
  if (info.log) {
    ...
    if (0 <= info.log.indexOf(") not found") && info.log.startsWith("Tx (")) {
      return errors.createError("transaction not found", errors.NOT_FOUND, {
        operation: method,
        info,
        response: result,
        params,
    ...
    // Height must be less than or equal to the current protocol height
    ...
    if (
      ...
      0 <= info.log.indexOf("Could not find results for height #")
    ) {
      return errors.createError("block not found", errors.NOT_FOUND, {
        operation: method,
        info,
        response: result,
        params,
