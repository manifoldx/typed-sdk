...
export class KeyPair {
  ...
  constructor(privateKey: Arrayish | string) {
    ...
    defineReadOnly(
      this,
      "publicKeyBytes",
      keyPair.getPublic().encode(null, true)
    );
  }
  ...
  computeSharedSecret(otherKey: Arrayish | string): string {
    let keyPair = getCurve().keyFromPrivate(arrayify(this.privateKey));
    let otherKeyPair = getCurve().keyFromPublic(
...
export function computePublicKey(
  key: Arrayish | string,
  compressed?: boolean
): string {
...
export function computeAddress(
  ...
  if (typeof key == "string") {
    if (
      key.match(/^(0x)?[0-9a-fA-F]{40}$/) ||
      key.match(/^(0x)?[0-9a-fA-F]{64}$/)
    ) {
  ...
  return getAddress(
    bech32Encode(prefix ? prefix : AddressPrefix, bech32ToWords(bytes))
  );
}
...
export function recoverAddress(
  ...
  return computeAddress(
    recoverPublicKey(arrayify(digest), signature, recoveryParam)
  );
}
