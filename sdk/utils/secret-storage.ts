...
export function decrypt(json: string, password: Arrayish, progressCallback?: ProgressCallback): Promise<SigningKey> {
    ...
    var getSigningKey = function (key: Uint8Array, reject: (error?: Error) => void) {
        ...
        if (searchPath(data, 'x-manifold/version') === '0.1') {
            var mnemonicCiphertext = looseArrayify(searchPath(data, 'x-manifold/mnemonicCiphertext'));
            var mnemonicIv = looseArrayify(searchPath(data, 'x-manifold/mnemonicCounter'));
