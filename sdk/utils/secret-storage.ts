...
export function decrypt(
  ...
  var getSigningKey = function (
    ...
    if (cipher === "aes-256-ctr") {
      ...
      computedMAC = hexlify(
        computeMAC(key.slice(32, 48), ciphertext)
      ).substring(2);
    ...
    // Version 0.1 x-manifold metadata must contain an encrypted mnemonic phrase
    if (searchPath(data, "x-manifold/version") === "0.1") {
      var mnemonicCiphertext = looseArrayify(
        searchPath(data, "x-manifold/mnemonicCiphertext")
      );
      var mnemonicIv = looseArrayify(
        searchPath(data, "x-manifold/mnemonicCounter")
      );
      ...
      var path = searchPath(data, "x-manifold/path") || HDNode.defaultPath;
      var entropy = arrayify(mnemonicAesCtr.decrypt(mnemonicCiphertext));

      var locale = searchPath(data, "x-manifold/locale") || undefined;
      var mnemonic = HDNode.entropyToMnemonic(entropy, locales[locale]);

      var node = HDNode.fromMnemonic(mnemonic, locales[locale]).derivePath(
        path
      );
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (kdf && typeof kdf === "string") {
      if (kdf.toLowerCase() === "scrypt") {
        ...
        scrypt(passwordBytes, salt, N, r, p, dkLen + 32, function (
          error,
          progress,
          key
        ) {
      ...
      if (error) {
        ...
        if (entropy) {
          ...
          data["x-manifold"] = {
            ...
            path: path,
            locale:
              options && options.locale ? options.locale.locale : undefined,
            version: "0.1",
          };
        }
