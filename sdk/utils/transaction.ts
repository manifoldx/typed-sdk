...
function handleNumber(value: string): number {
  if (value === "0x") {
    return bigNumberify(value).toNumber();
  }
...
export function serialize(
  ...
  signature?: Arrayish | Signature,
  publicKey?: string
): string {
  checkTransaction(unsignedTransaction);

  if (!signature) {
    return base64Encode(toUtf8Bytes(JSON.stringify(unsignedTransaction)));
  }
  ...
  if (!unsignedTransaction.value)
    return errors.throwError(
      ...
      { arg: "unsignedTransaction", value: unsignedTransaction }
    );
  ...
  transactionFields.forEach(function (fieldInfo) {
    ...
    transaction[fieldInfo.name] =
      "object" === typeof value ? shallowCopy(value) : value;
  });

  if (!transaction.value.signatures) transaction.value.signatures = [];
  ...
  transaction.value.signatures.push(<any>{
    ...
    signature: isArrayish(signature)
      ? signature
      : base64Encode(joinSignature(signature)),
  });
  ...
  switch (moduleName) {
    ...
    default:
      errors.throwError(
        ...
        { route, transactionType, overrides }
      );
  }
  return transaction;
}
