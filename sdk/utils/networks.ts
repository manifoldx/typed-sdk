...
function manifoldDefaultProvider(network: string): (providers: any) => any {
  return function (providers: any): any {
    ...
    providerList.push(
      new providers.JsonRpcProvider(
        "https://to.be.define.manifoldfinance.com",
        network
      )
    );
...
const homestead: Network = {
  chainId: "manifold",
  name: "manifold",
  _defaultProvider: manifoldDefaultProvider("homestead"),
};

const testnet: Network = {
  chainId: "evo protocol",
  name: "evo protocol",
  _defaultProvider: etcDefaultProvider(
    "https://evo protocol-rpc.manifoldfinance.com",
    "evo protocol"
  ),
};
...
export function getNetwork(network: Networkish): Network {
  ...
  if (
    isUndefinedOrNullOrEmpty(network.chainId) &&
    network.chainId !== n.chainId
  ) {
