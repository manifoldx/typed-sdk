...
export class Wallet extends AbstractSigner {
  ...
  get hexAddress(): string {
    return this.signingKey.hexAddress;
  }
  ...
  get wordlist(): Wordlist {
    return this.signingKey.wordlist;
  }
  get path(): string {
  ...
  get publicKey(): string {
    return this.signingKey.compressedPublicKey;
  }
  get publicKeyType(): string {
    return this.signingKey.publicKeyType;
  }
  ...
  sign(transaction: TransactionRequest, overrides?: any) {
    if (
      ...
      if (transaction.chainId == null) {
        transaction.chainId = this.provider
          .getNetwork()
    ...
    return resolveProperties(transaction).then((tx) => {
      if (
        ...
        !tx.value ||
        !tx.value.msg ||
        !Array.isArray(tx.value.msg)
      ) {
        errors.throwError(
          "missing transaction field",
          errors.MISSING_ARGUMENT,
          { argument: "value", value: tx }
        );
      }
      ...
      let signedTransaction = serializeTransaction(
        tx,
        signature,
  ...
  signMessage(message: Arrayish | string, excludeRecoveryParam?: boolean) {
    return Promise.resolve(
      joinSignature(
        ...
        excludeRecoveryParam ? false : true
      )
    );
  }
  ...
  getAccountNumber(blockTag?: BlockTag) {
    ...
    if (!this.accountNumber) {
      return this.provider
        .getAccountNumber(this.address, blockTag)
        .then((accountNumber) => {
      ...
      return this.sendTransaction(tx, overrides).then((response) => {
        ...
        return this.provider
          ...
          .then((receipt) => {
            ...
            throw this.provider.checkTransactionReceipt(
              receipt,
              errors.CALL_EXCEPTION,
              "transfer failed",
              {
                method: "manifold/msgSend",
                receipt,
              }
            );
          });
  ...
  getTransferTransactionRequest(
    ...
    return this.provider.resolveName(addressOrName).then((address) => {
      let tx = this.provider.getTransactionRequest("bank", "bank-send", {
        ...
        denom:
          overrides && overrides.denom ? overrides.denom : smallestUnitName,
      });
      tx.fee =
  ...
  createAlias(
    ...
    appFee: { to: string; value: BigNumberish },
    overrides?: any
  ): Promise<TransactionResponse | TransactionReceipt> {
    return this.getCreateAliasTransactionRequest(name, appFee, overrides).then(
      (tx) => {
        return this.sendTransaction(tx, overrides).then((response) => {
          if (overrides && overrides.sendOnly) {
            return response;
  ...
  getCreateAliasTransactionRequest(
    name: string | Promise<string>,
    appFee: { to: string; value: BigNumberish },
    overrides?: any
  ): Promise<TransactionRequest> {
    ...
    if (bigNumberify(appFee.value).lte(0)) {
      errors.throwError(
        "create alias transaction require non-zero application fee",
        errors.MISSING_FEES,
        { value: appFee }
      );
    }
  ...
  encrypt(
    ...
    if (progressCallback && typeof progressCallback !== "function") {
      errors.throwError("invalid callback", errors.INVALID_ARGUMENT, {
        argument: "progressCallback",
      });
    }
  ...
  static createRandom(options?: any): Wallet {
    ...
    if (options.extraEntropy) {
      entropy = arrayify(
        sha256(concat([entropy, options.extraEntropy])).substring(0, 34)
      );
    }
  ...
  static fromEncryptedJson(
    ...
    return Promise.reject(
      errors.createError("invalid wallet JSON", errors.UNEXPECTED_ARGUMENT, {})
    );
  }
