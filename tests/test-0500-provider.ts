...
describe('Suite: Provider', function () {
    ...
    provider = new manifold.providers.JsonRpcProvider(nodeProvider.connection, nodeProvider).on("rpc", function (args) {
        if (!silentRpc) {
    ...
    it("Get account state", function () {
        return provider.getAccountState("manifold1mklypleqjhemrlt6z625rzqa0jl6namdmmqnx4", null).then((state) => {
            if (!silent) console.log(indent, "Account state:", JSON.stringify(state));
        });
    });

    it("Get balance", function () {
        return provider.getBalance("manifold1mklypleqjhemrlt6z625rzqa0jl6namdmmqnx4", null).then((balance) => {
    ...
    it("Get account number with non-exists address", function () {
        return provider.getAccountNumber("manifold18nyjc5sxz0tlndf8uslyj6k9leha59vc23djl6").then((accountNumber) => {
    ...
    it("Get alias application", function () {
        return provider.getAliasState("manifold1y5vnkav6aara3a6srruu3u5v7rhke9n7unxhda").then((state) => {
