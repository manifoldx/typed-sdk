...
let provider: manifold.providers.Provider;
...
describe("Suite: Provider", function () {
  ...
  provider = new manifold.providers.JsonRpcProvider(
  ...
  it("Get account state", function () {
    return provider
      .getAccountState("manifold1mklypleqjhemrlt6z625rzqa0jl6namdmmqnx4", null)
      .then((state) => {
        if (!silent)
          console.log(indent, "Account state:", JSON.stringify(state));
      });
  });

  it("Get balance", function () {
    return provider
      .getBalance("manifold1mklypleqjhemrlt6z625rzqa0jl6namdmmqnx4", null)
      .then((balance) => {
  ...
  it("Get block details", function () {
    return provider.getBlock("100").then((block) => {
      ...
      if (!silent)
        console.log(indent, "Block:", JSON.stringify(block, null, 2));
  ...
  it("Get transaction", function () {
    return provider
      .getTransaction(
        "0x249dcf0fdb6937e85bbdd729609fbc01751592e96c4ab46de764538e12fd9ae9"
      )
      .then((transaction) => {
        if (!silent)
          console.log(
  ...
  it("Lookup address", function () {
    let address = "manifold1aef2hwa64ffea5vmn9uqz55j8gvjkta99fpkch";
  ...
  it("Get account number with non-exists address", function () {
    return provider
      .getAccountNumber("manifold18nyjc5sxz0tlndf8uslyj6k9leha59vc23djl6")
      .then((accountNumber) => {
        expect(accountNumber).to.exist;
        expect(accountNumber.toString()).to.equal("0");
        if (!silent)
          console.log(indent, "AccountNumber:", accountNumber.toString());
      });
  });

  it("Check KYC whitelist status", function () {
    let address = "manifold1wv3kquk24x8lh06905z98r9wydzu6e55lfdeae";
  ...
  it("Get transaction fee", function () {
    return provider
      .getTransactionFee("bank", "bank-send", {
        from: "manifold1qj75krmwsug5le85quhxjmy4pjj2uh9mmyaqdv",
        to: "manifold1qj75krmwsug5le85quhxjmy4pjj2uh9mmyaqdv",
        value: "1000000000",
  ...
  it("Get transaction fee setting", function () {
    return provider.getTransactionFeeSetting("bank-send").then((setting) => {
      ...
      if (!silent)
        console.log(indent, "transactionFeeSetting:", JSON.stringify(setting));
    });
  });

  it("Get alias application", function () {
    return provider
      .getAliasState("manifold1y5vnkav6aara3a6srruu3u5v7rhke9n7unxhda")
      .then((state) => {
        if (!silent)
          console.log(
            indent,
            "Alias state:",
            state ? JSON.stringify(state) : "<EMPTY>"
          );
      });
  });
