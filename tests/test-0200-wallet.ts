...
import { progress, clearLine } from "./utils-manifold";
import { manifold, errors, wordlists, utils } from "../src.ts/index";
import { formatMan, base64, toUtf8String, toUtf8Bytes } from "../src.ts/utils";
...
let wallet1: manifold.Wallet;
let wallet2: manifold.Wallet;
let wallet3: manifold.Wallet;
let walletAirDrop: manifold.Wallet;
  ...
  describe("Suite: Wallet", function () {
    ...
    it("CreateRandom with different number of words", function () {
      for (let n = 16; 32 >= n; n += 4) {
        ...
        let anotherWallet = manifold.Wallet.fromMnemonic(wallet.mnemonic);
    ...
    it("CreateRandom with multi languages", function () {
      for (let language of Object.keys(wordlists.locales)) {
        wallet = manifold.Wallet.createRandom({
          locale: wordlists.locales[language],
        });
        expect(wallet).to.exist;
        if (!silent)
          console.log(indent, "Wallet -", language, ":", wallet.mnemonic);
      }
    });
    ...
    it("CreateRandom with multi languages and different number of words", function () {
      for (let language of Object.keys(wordlists.locales)) {
        for (let n = 16; 32 >= n; n += 4) {
          ...
          let anotherWallet = manifold.Wallet.fromMnemonic(
            wallet.mnemonic,
            null,
            wordlists.locales[language]
          );
          expect(anotherWallet).to.exist;
          expect(anotherWallet.address).to.equal(wallet.address);
    ...
    it("Create from mnemonic", function () {
      wallet1 = manifold.Wallet.fromMnemonic(nodeProvider.kyc.issuer);
      expect(wallet1).to.exist;
      if (!silent) console.log(indent, "Wallet:", JSON.stringify(wallet1));

      walletAirDrop = manifold.Wallet.fromMnemonic(nodeProvider.airDrop);
    });
        ...
        for (let n = 16; 32 >= n; n += 4) {
          for (let i = 0; 10 > i; i++) {
            ...
            if (!silent) {
              let words = wordlists.locales[language].split(wallet.mnemonic);
              let message =
                words.length + " " + language + ": " + wallet.mnemonic;
              clearLine();
    ...
    it("Transfer", function () {
      let value = manifold.utils.parseMan("100");
      let overrides = {
      ...
      return wallet.provider
        .getTransactionFee("bank", "bank-send", {
          from: wallet.address,
        ...
        .then((fee) => {
          overrides["fee"] = fee;
          return wallet
            .transfer(walletAirDrop.address, value, overrides)
            .then((receipt) => {
      ...
      return walletAirDrop.provider
        ...
        .then((fee) => {
          ...
          return walletAirDrop
            .transfer(wallet.address, value, overrides)
            .then((receipt) => {
              expect(receipt).to.exist;
        ...
        .then((fee) => {
          ...
          return wallet
            ...
            .then((signedTransaction) => {
              return providerConnection.sendTransaction(
                signedTransaction,
                overrides
              );
            })
    ...
    it("Sign message", function () {
      ...
      return wallet.signMessage(message).then((signature) => {
        ...
        let valid = manifold.utils.verify(message, signature, wallet.address);
    ...
    it("Sign transaction", function () {
      ...
      return walletAirDrop.provider
        ...
        .then((fee) => {
          let transaction = wallet.provider.getTransactionRequest(
            "bank",
            "bank-send",
            {
              from: wallet.address,
      ...
      return walletAirDrop.provider
        ...
        .then((transaction) => {
          return wallet
            ...
            .then((signedTransaction) => {
              let tx = JSON.parse(
                toUtf8String(base64.decode(signedTransaction))
              );
              if (!silent)
                console.log(indent, "Transaction:", JSON.stringify(tx));

              // Inject anonymous attributes after signed to avoid invalid signature
              tx.value.msg[0].value["invisible"] = "I am invisible";
              signedTransaction = base64.encode(
                toUtf8Bytes(JSON.stringify(tx))
              );

              return wallet.provider.sendTransaction(
                signedTransaction,
                overrides
              );
            })
            .then((response) => {
              ...
              if (!silent)
                console.log(indent, "Response:", JSON.stringify(response));
              ...
              return wallet.provider
                ...
                .then((receipt) => {
                  ...
                  expect(receipt.payload.value.msg[0].value["invisible"]).to.not
                    .exist;
                  if (!silent)
    ...
    it("Encrypt wallet with multi-language", function () {
      ...
      if (!silent)
        console.log(
          indent,
          "Encrypt wallet with",
          Object.keys(wordlists.locales).length,
          "languages"
        );
      let promises = Promise.resolve();
      for (let locale of Object.keys(wordlists.locales)) {
        let wallet = manifold.Wallet.createRandom({
          locale: wordlists.locales[locale],
        });
        expect(wallet).to.exist;
        ...
        let progressCallback = (percentage) => {
          ...
          if (!silent)
            progress(
              ++counter,
              percentage,
              "Encrypting (" + locale + "): " + wallet.mnemonic,
              6
            );
        };
        promises = promises.then(() => {
          return wallet
            .encrypt("any strong password", progressCallback)
            .then((encrypted) => {
    ...
    it("Decrypt wallet with multi-language", function () {
      ...
      for (let encryptedWallet of encryptedWallets) {
        ...
        promises = promises.then(() => {
          return manifold.Wallet.fromEncryptedJson(
            encryptedWallet,
            "any strong password",
            progressCallback
          ).then((wallet) => {
    ...
    it("Load encrypted wallet format v0.1", function () {
      let encryptedWallet =
        '{"address":"manifold1m5qg7swt85y7h90evp75h48ah7kq4d9qnshlg5","id":"9fb5b452-5d8e-4f21-b86c-36c8fb89a6ee","version":3,"Crypto":{"cipher":"aes-128-ctr","cipherparams":{"iv":"6225d04b84258963be446d6f2827c083"},"ciphertext":"ba4ab50b3811700eb13a6ce20210b14b7e3570e07b46add6b254c6f81e04b044","kdf":"scrypt","kdfparams":{"salt":"3a1cba8bd23170d41193da67682484cdd5afc639a7359dd119794d8c577e9a6f","n":131072,"dklen":32,"p":1,"r":8},"mac":"ac78dd013c91dfaf905f2185401f4bf9fd132d1fa5fbbb729e822e761bfaa2ec"},"x-manifold":{"client":"manifold-sdk","filename":"UTC--2020-07-19T17-25-11.0Z--manifold1m5qg7swt85y7h90evp75h48ah7kq4d9qnshlg5","mnemonicCounter":"1cfaee39612c2f4361f52c2fbfcd8ab0","mnemonicCiphertext":"b495932d586c6de3d8dde92f74416218","path":"m/44\'/376\'/0\'/0/0","version":"0.1"}}';
      ...
      return manifold.Wallet.fromEncryptedJson(
        ...
        expect(wallet)
          ...
          .is.eq("manifold1m5qg7swt85y7h90evp75h48ah7kq4d9qnshlg5");
        expect(wallet)
          .has.property("mnemonic")
          .is.eq(
            "ask cliff eyebrow sound age lottery initial casual choose book pudding invest"
          );
      });
    });
    ...
    it("Load encrypted wallet format v0.2", function () {
      let encryptedWallet =
        '{"address":"manifold155uygslajet0ppga72vgah36kryuta6wr9233g","id":"c222613a-4058-46cb-9a2e-d73f0a3b4320","version":3,"Crypto":{"cipher":"aes-256-ctr","cipherparams":{"iv":"b53f720ded76b0cf156ccdeb9de52501"},"ciphertext":"7b93e5ae431c75bf3ccc103f657864c1cbc884bf51979e62c18b159270fcc25f","kdf":"scrypt","kdfparams":{"salt":"16015290370acbaba7bca00a5116414d9d0380b10e0b3f69e13222fbce7656a6","n":131072,"dklen":48,"p":1,"r":8},"mac":"d93a0277d435b419c5d0dbd6cef5d78f6b8bd8147e4739d3408a9a6c41b16470"},"x-manifold":{"client":"manifold-sdk","filename":"UTC--2020-07-19T17-42-56.0Z--manifold155uygslajet0ppga72vgah36kryuta6wr9233g","mnemonicCounter":"ed455dd20df0968174b9dd0103589e72","mnemonicCiphertext":"07f561c571747588ce803b7496519382","path":"m/44\'/376\'/0\'/0/0","locale":"en","version":"0.1"}}';
      ...
      return manifold.Wallet.fromEncryptedJson(
        ...
        expect(wallet)
          .has.property("address")
          .is.eq("manifold155uygslajet0ppga72vgah36kryuta6wr9233g");
        expect(wallet)
          .has.property("mnemonic")
          .is.eq(
            "what bike cactus chalk hole height skill lava owner student festival trap"
          );
      });
    });
    ...
    it("Compute shared secret", function () {
      ...
      if (!silent)
        console.log(indent, "Shared secret wallet 1 with 2:", secret1With2);
      if (!silent)
        console.log(indent, "Shared secret wallet 2 with 1:", secret2With1);

      if (!silent)
        console.log(
          indent,
          utils.computeAddress(wallet1.publicKey),
          "publicKey:",
          wallet1.publicKey
        );
      if (!silent)
        console.log(
          indent,
          utils.computeAddress(wallet2.publicKey),
          "publicKey:",
          wallet2.publicKey
        );
    ...
    it("Verify mnemonic", function () {
      let wallet = manifold.Wallet.fromMnemonic(
        "lady rebel cash silent object vault peace deal forward problem guide number"
      );
      expect("manifold1lgaaw7r5nw49d70vgm79k9hjjdpcrhqgum0vmm").to.equal(
        wallet.address
      );
    });
  });
