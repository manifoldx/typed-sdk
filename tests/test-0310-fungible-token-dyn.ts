...
import { manifold, token, errors } from "../src.ts/index";
...
if ("" != nodeProvider.fungibleToken.middleware) {
  describe("Suite: FungibleToken - Dynamic Supply", function () {
    ...
    it("Initialize", function () {
      ...
      provider = manifold.Wallet.fromMnemonic(
        nodeProvider.fungibleToken.provider
      ...
      issuer = manifold.Wallet.fromMnemonic(
        nodeProvider.fungibleToken.issuer
      ).connect(providerConnection);
      ...
      if (!silent)
        console.log(
          indent,
          "Fee collector:",
          JSON.stringify({ address: nodeProvider.fungibleToken.feeCollector })
        );
    });
  });
  ...
  [true, false].forEach((unlimited) => {
    describe(
      "Suite: FungibleToken - Dynamic Supply " +
      ...
      function () {
        ...
        it("Create - challenge negative application fee", function () {
          ...
          let fungibleTokenProperties = {
            ...
            maxSupply: unlimited
              ? bigNumberify("0")
              : bigNumberify("100000000000000000000000000"),
            fee: {
        ...
        it("Create - checkDuplication", function () {
          return token.FungibleToken.create(fungibleTokenProperties, issuer)
            .then((token) => {
              expect(token).is.not.exist;
            })
            .catch((error) => {
          ...
          return refresh(fungibleTokenProperties.symbol).then(() => {
            ...
            if (!silent)
              console.log(
                indent,
                "Created Token:",
                JSON.stringify(fungibleToken.state)
              );
          });
        });
        ...
        it("Approve - challenge wrong fee setting", function () {
          let overrides = {
            tokenFees: [
              ...
              {
                action: FungibleTokenActions.acceptOwnership,
                feeName: "default",
              },
            ],
            burnable: true,
          };
          return performFungibleTokenStatus(
            fungibleTokenProperties.symbol,
        ...
        it("State", function () {
          return issuerFungibleToken.getState().then((state) => {
            ...
            expect(state)
              .have.property("name")
              .is.eq(fungibleTokenProperties.name);
            expect(state)
              .have.property("symbol")
              .is.eq(fungibleTokenProperties.symbol);
            expect(state)
              .have.property("decimals")
              .is.eq(fungibleTokenProperties.decimals);
            expect(state)
            ...
            expect(state)
              .have.property("maxSupply")
              .is.satisfy(function (value) {
                return value.eq(fungibleTokenProperties.maxSupply);
              });
        ...
        it("Balance - owner", function () {
          return issuerFungibleToken.getBalance().then((balance) => {
            expect(balance.toString()).to.equal(
              issuerFungibleToken.state.totalSupply.toString()
            );
            if (!silent)
              console.log(
                indent,
                "Owner balance:",
                manifold.utils.formatUnits(
                  balance,
                  issuerFungibleToken.state.decimals
                ),
                issuerFungibleToken.symbol
              );
          });
        });
        ...
        it("Mint", function () {
          let value = unlimited
            ? bigNumberify("100000000000000000000000000")
            : issuerFungibleToken.state.maxSupply;
          return issuerFungibleToken
            .mint(issuer.address, value)
            .then((receipt) => {
              expect(receipt).have.property("status").is.eq(1);
              if (!silent)
                console.log(indent, "RECEIPT:", JSON.stringify(receipt));
        ...
        it("Transfer - self transfer", function () {
          return issuerFungibleToken.getBalance().then((balance) => {
            return issuerFungibleToken
              .transfer(issuer.address, balance)
              .then((receipt) => {
                expect(receipt).have.property("status").is.eq(1);
                if (!silent)
                  console.log(indent, "RECEIPT:", JSON.stringify(receipt));
        ...
        it("Transfer", function () {
          return issuerFungibleToken.getBalance().then((balance) => {
            return wallet.provider
              .getTransactionFee("token", "token-transferFungibleToken", {
              ...
              .then((fee) => {
                return issuerFungibleToken
                  .transfer(wallet.address, balance, { fee })
                  .then((receipt) => {
                    expect(receipt).have.property("status").is.eq(1);
                    if (!silent)
                      console.log(indent, "RECEIPT:", JSON.stringify(receipt));
        ...
        it("Mint - after unfreeze token", function () {
          ...
          return issuerFungibleToken
            ...
            .then((receipt) => {
              expect(receipt).have.property("status").is.eq(1);
              if (!silent)
                console.log(indent, "Mint RECEIPT:", JSON.stringify(receipt));
            });
        });
        ...
        it("Transfer - after unfreeze token", function () {
          return issuerFungibleToken.getBalance().then((balance) => {
            balance = balance.div(2);
            return issuerFungibleToken
              .transfer(wallet.address, balance)
              .then((receipt) => {
                expect(receipt).have.property("status").is.eq(1);
                if (!silent)
                  console.log(indent, "RECEIPT:", JSON.stringify(receipt));
        ...
        it("Freeze Account", function () {
          return performFungibleTokenAccountStatus(
            fungibleTokenProperties.symbol,
            wallet.address,
            token.FungibleToken.freezeFungibleTokenAccount
          ).then((receipt) => {
            expect(receipt).have.property("status").is.eq(1);
            if (!silent)
              console.log(indent, "RECEIPT:", JSON.stringify(receipt));
        ...
        it("Unfreeze Account", function () {
          return performFungibleTokenAccountStatus(
            fungibleTokenProperties.symbol,
            wallet.address,
            token.FungibleToken.unfreezeFungibleTokenAccount
          ).then((receipt) => {
            expect(receipt).have.property("status").is.eq(1);
            if (!silent)
              console.log(indent, "RECEIPT:", JSON.stringify(receipt));
        ...
        it("Approve transfer ownership", function () {
          return performFungibleTokenStatus(
            fungibleTokenProperties.symbol,
            token.FungibleToken.approveFungibleTokenOwnership
          ).then((receipt) => {
            expect(receipt).have.property("status").is.eq(1);
            if (!silent)
              console.log(indent, "RECEIPT:", JSON.stringify(receipt));
    ...
    describe(
      "Suite: FungibleToken - Dynamic Supply " +
        (unlimited ? "(Unlimited - Reject)" : "(Limited - Reject)"),
      function () {
        this.slow(slowThreshold); // define the threshold for slow indicator
        ...
        it("Reject", function () {
          ...
          return performFungibleTokenStatus(
            fungibleTokenProperties.symbol,
            token.FungibleToken.rejectFungibleToken,
            overrides
          ).then((receipt) => {
            expect(receipt).have.property("status").is.eq(1);
            if (!silent)
        ...
        it("Status", function () {
          ...
          return token.FungibleToken.fromSymbol(symbol, wallet)
            .then((token) => {
              expect(token).is.not.exist;
            })
            .catch((error) => {
  ...
  function performFungibleTokenStatus(
    symbol: string,
    perform: any,
    overrides?: any
  ) {
  ...
  function performFungibleTokenAccountStatus(
    symbol: string,
    target: string,
    perform: any,
    overrides?: any
  ) {
    return perform(symbol, target, provider, overrides)
      .then((transaction) => {
        return token.FungibleToken.signFungibleTokenAccountStatusTransaction(
          transaction,
          issuer
        );
      })
      .then((transaction) => {
        return token.FungibleToken.sendFungibleTokenAccountStatusTransaction(
          transaction,
          middleware
        ).then((receipt) => {
          expect(receipt).have.property("status").is.eq(1);
  ...
  function refresh(symbol: string) {
    return token.FungibleToken.fromSymbol(symbol, wallet)
      .then((token) => {
        ...
        fungibleToken = token;
        if (!silent)
          console.log(indent, "STATE:", JSON.stringify(fungibleToken.state));
      })
      .then(() => {
