...
if ("" != nodeProvider.fungibleToken.middleware) {
    describe('Suite: FungibleToken - Dynamic Supply', function () {
        ...
        it("Initialize", function () {
            ...
            wallet = manifold.Wallet.fromMnemonic(nodeProvider.kyc.issuer).connect(providerConnection);
            expect(wallet).to.exist;
            if (!silent) console.log(indent, "Wallet:", JSON.stringify({ address: wallet.address, mnemonic: wallet.mnemonic }));

            provider = manifold.Wallet.fromMnemonic(nodeProvider.fungibleToken.provider).connect(providerConnection);
            expect(provider).to.exist;
            if (!silent) console.log(indent, "Provider:", JSON.stringify({ address: provider.address, mnemonic: provider.mnemonic }));

            issuer = manifold.Wallet.fromMnemonic(nodeProvider.fungibleToken.issuer).connect(providerConnection);
            expect(issuer).to.exist;
            if (!silent) console.log(indent, "Issuer:", JSON.stringify({ address: issuer.address, mnemonic: issuer.mnemonic }));

            middleware = manifold.Wallet.fromMnemonic(nodeProvider.fungibleToken.middleware).connect(providerConnection);
    ...
    [true, false].forEach((unlimited) => {
        describe('Suite: FungibleToken - Dynamic Supply ' + (unlimited ? "(Unlimited)" : "(Limited)"), function () {
            ...
            it("AccountState - owner", function () {
                let provider = new manifold.providers.JsonRpcProvider(nodeProvider.connection, nodeProvider);
            ...
            it("Balance - owner", function () {
                return issuerFungibleToken.getBalance().then((balance) => {
                    ...
                    if (!silent) console.log(indent, "Owner balance:", manifold.utils.formatUnits(balance, issuerFungibleToken.state.decimals), issuerFungibleToken.symbol);
                });
            });
            ...
            it("Balance - others", function () {
                ...
                return provider.getTokenAccountBalance(fungibleTokenProperties.symbol, wallet.address).then((balance) => {
                    ...
                    if (!silent) console.log(indent, "Other's balance:", manifold.utils.formatUnits(balance, issuerFungibleToken.state.decimals), issuerFungibleToken.symbol);
                });
            });
            ...
            it("Balance - total supply", function () {
                return issuerFungibleToken.getBalance().then((balance) => {
                    expect(balance.toString()).to.equal(issuerFungibleToken.state.totalSupply.toString());
                    if (!silent) console.log(indent, "Owner balance:", manifold.utils.formatUnits(balance, issuerFungibleToken.state.decimals));
                });
            });
