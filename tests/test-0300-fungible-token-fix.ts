...
if ("" != nodeProvider.fungibleToken.middleware) {
  ...
  [false, true].forEach((burnable) => {
    describe(
      "Suite: FungibleToken - Fixed Supply " +
        (burnable ? "(Burnable)" : "(Not Burnable)"),
      function () {
        this.slow(slowThreshold); // define the threshold for slow indicator
        ...
        it("Create - checkDuplication with manual broadcast", function () {
          return token.FungibleToken.getCreateTransactionRequest(
            fungibleTokenProperties,
            issuer,
            defaultOverrides
          )
            ...
            .then((receipt) => {
              return token.FungibleToken.fromSymbol(
        ...
        it(
          "Burn" + (burnable ? "" : " - challenge non-burnable token"),
          function () {
            return issuerFungibleToken
              .getBalance()
              .then((balance) => {
