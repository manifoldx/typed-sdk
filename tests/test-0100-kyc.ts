  ...
  describe("Suite: KYC", function () {
    ...
    it("Initialize", async function () {
      ...
      if (!silent)
        console.log(
          indent,
          "Provider:",
          await provider.encrypt("any strong password")
        );
      ...
      if (!silent)
        console.log(
          indent,
          "Issuer:",
          await issuer.encrypt("any strong password")
        );
      ...
      if (!silent)
        console.log(
          indent,
          "Middleware:",
          await middleware.encrypt("any strong password")
        );
      ...
      for (let i = 0; i < walletCount; i++) {
        ...
        wallets.push(wallet);
        if (!silent)
          console.log(indent, "Wallet:", wallet.address, "-", wallet.mnemonic);
      }
    });
    ...
    it("Sign kyc address", function () {
      ...
      for (let wallet of wallets) {
        promises.push(
          auth.Kyc.create(wallet).then((kyc) => {
            let seed = sha256(
              toUtf8Bytes(
                JSON.stringify(
                  sortObject({
            ...
            return kyc.sign(kycAddress).then((data) => {
              ...
              if (!silent)
                console.log(indent, "KYC data:", JSON.stringify(data));
            });
          })
        );
      }
      ...
      for (let kycTransaction of kycTransactions) {
        if (!silent)
          console.log(
            indent,
            "Kyc transaction:",
            JSON.stringify(kycTransaction)
          );
        let result = manifold.utils.verify(
          JSON.stringify(kycTransaction.payload.kyc),
          kycTransaction.payload.signature,
          kycTransaction.payload.kyc.from
        );
        expect(true).to.equal(result);

        for (let signature of kycTransaction.signatures) {
          let address = computeAddress(signature.pub_key.value);
          let valid = manifold.utils.verify(
            JSON.stringify(kycTransaction.payload),
            signature.signature,
            address
          );
          if (!valid) {
    ...
    it("Whitelist", function () {
      return auth.Kyc.create(middleware).then(async (kyc) => {
        for (let kycTransaction of kycTransactions) {
          ...
          if (!silent)
            console.log(indent, "Whitelist.receipt:", JSON.stringify(receipt));
    ...
    it("Whitelist - checkDuplication with manual broadcast", function () {
      return auth.Kyc.create(middleware).then(async (kyc) => {
        for (let kycTransaction of kycTransactions) {
          ...
          try {
            let request = await kyc.getWhitelistTransactionRequest(
              kycTransaction,
              overrides
            );
            let signedTransaction = await middleware.sign(request, overrides);
            let response = await providerConnection.sendTransaction(
              signedTransaction,
              overrides
            );
            let receipt = await providerConnection.waitForTransaction(
              response.hash
            );
    ...
    it("Check KYC address", async function () {
      ...
      for (let wallet of wallets) {
        ...
        if (!silent)
          console.log(indent, "Address:", wallet.address, "-", kycAddress);
    ...
    it("KYC bind wallets relationship", async function () {
      for (let wallet of wallets) {
        let kycAddress = await wallet.provider.getKycAddress(wallet.address);
        let receipt = await auth.Kyc.bind(
          wallet.address,
          kycAddress,
          middleware
        );
        expect(receipt).to.exist;
        if (!silent)
          console.log(indent, "kycBind.receipt:", JSON.stringify(receipt));
        expect(receipt.status).to.equal(1);
      }
    });
    ...
    it("KYC unbind wallets relationship", async function () {
      for (let wallet of wallets) {
        let kycAddress = await wallet.provider.getKycAddress(wallet.address);
        let receipt = await auth.Kyc.unbind(
          wallet.address,
          kycAddress,
          middleware
        );
        expect(receipt).to.exist;
        if (!silent)
          console.log(indent, "kycUnbind.receipt:", JSON.stringify(receipt));
        expect(receipt.status).to.equal(1);
      }
    });
    ...
    it("Revocation", async function () {
      for (let wallet of wallets) {
        await auth.Kyc.revoke(wallet.address, provider)
          .then((transaction) => {
            expect(transaction).to.exist;
            if (!silent)
              console.log(
                indent,
                "Provider signed revoke transaction:",
          ...
          .then((transaction) => {
            ...
            if (!silent)
              console.log(
                indent,
                "Issuer signed revoke transaction:",
                JSON.stringify(transaction)
              );
            return auth.Kyc.sendRevokeTransaction(
              transaction,
              middleware,
              defaultOverrides
            );
          ...
          .then((receipt) => {
            expect(receipt).to.exist;
            if (!silent)
              console.log(indent, "Revoke RECEIPT:", JSON.stringify(receipt));
            expect(receipt.status).to.equal(1);
          });
      }
    });
