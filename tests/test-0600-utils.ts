...
describe("Suite: Utils", function () {
  ...
  it("Test unit convert to protocol format (readable => bigNumber)", function () {
    let readable = "12345678.90";
    let bn = utils.parseMan(readable);
    if (!silent)
      console.log(indent, "BigNumber:", readable, "=>", bn.toString());
    expect(bn.toString()).to.equal("12345678900000000000000000");
  });

  it("Test unit convert to protocol format with 18 decimals (readable => bigNumber)", function () {
    let readable = "12345678.90";
    let converted = utils.parseUnits(readable, 18);
    if (!silent)
      console.log(indent, "BigNumber:", readable, "=>", converted.toString());
  ...
  it("Test unit convert to readable format (bigNumber => readable)", function () {
    ...
    let converted = utils.formatMan(value);
    if (!silent)
      console.log(indent, "Readable:", value.toString(), "=>", converted);
  ...
  it("Validate address", function () {
    let address = "manifold1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqgcpfl3";

    let hexAddress = utils.computeHexAddress(address);
    expect(hexAddress).to.be.equal(
  ...
  it("Convert hex address", function () {
    ...
    expect(address).to.be.equal(
      "manifold1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqgcpfl3"
    );
    if (!silent) console.log(indent, "Address:", address);
  });

  it("Normalize bech32 address", function () {
    let address = "manifold15f3aw83y2hgzzs6hk6utputr2xchwe5x745l9s";
  ...
  it("Test SHA256 hash value", function () {
    let hash = utils.sha256(toUtf8Bytes("Hello, protocol"));
    if (!silent) console.log(indent, hash);
    expect(
      "0x6cabdb6b53060a1961f7cceede7e07022505a83870fadb64f9699074150f033e"
    ).to.equal(hash);
  });
