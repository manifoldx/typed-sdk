...
import { manifold, nonFungibleToken as token, errors } from "../src.ts/index";
...
if ("" != nodeProvider.nonFungibleToken.middleware) {
  describe("Suite: NonFungibleToken - private", function () {
    ...
    it("Initialize", function () {
      ...
      issuer = manifold.Wallet.fromMnemonic(
      ...
      if (!silent)
        console.log(
          ...
          "Fee collector:",
          JSON.stringify({
            address: nodeProvider.nonFungibleToken.feeCollector,
          })
        );
    });
  });
            ...
            describe(
              ...
              function () {
                ...
                it("Create - challenge negative application fee", function () {
                  ...
                  return token.NonFungibleToken.create(
                  ...
                  return refresh(nonFungibleTokenProperties.symbol).then(() => {
                    ...
                    if (!silent)
                      console.log(
                        indent,
                        "Created Token:",
                        JSON.stringify(issuerNonFungibleToken.state)
                      );
                  });
                });
                ...
                it("Challenge update endorser list before token being approved", function () {
                  ...
                  return issuerNonFungibleToken
                    .updateEndorserList(endorsers)
                    .then((receipt) => {
                ...
                it("Approve - challenge wrong fee setting", function () {
                  let overrides = {
                    tokenFees: [
                      {
                        action: NonFungibleTokenActions.transfer,
                        feeName: "anything",
                      },
                      {
                      ...
                      {
                        action: NonFungibleTokenActions.endorse,
                        feeName: "default",
                      },
                      {
                        action: NonFungibleTokenActions.updateNFTEndorserList,
                        feeName: "default",
                      },
                    ],
                    endorserList: [],
                  ...
                  return performNonFungibleTokenStatus(
                    nonFungibleTokenProperties.symbol,
                ...
                if (0 < endorserListLimit) {
                  ...
                  it("State", function () {
                    return issuerNonFungibleToken.getState().then((state) => {
                      ...
                      expect(state)
                        ...
                        .is.eq("properties");
                      expect(state)
                        .have.property("endorserList")
                        .is.an("array")
                        .to.have.lengthOf(0);
                      expect(state)
                        .have.property("transferLimit")
                        .is.satisfy(function (value) {
                          return value.eq(1);
                        });
                      expect(state)
                        .have.property("mintLimit")
                        .is.satisfy(function (value) {
                          return value.eq(1);
                        });
                      expect(state)
                        .have.property("totalSupply")
                        .is.satisfy(function (value) {
                          return value.eq(0);
                        });
                      expect(state)
                        .have.property("endorserListLimit")
                        .is.satisfy(function (value) {
                          return value.eq(endorserListLimit);
                        });
                  ...
                  it("Transfer ownership - non-owner", function () {
                    return nonFungibleToken
                      .transferOwnership(issuer.address)
                      .then((receipt) => {
                        expect(receipt).is.not.exist;
                      })
                      .catch((error) => {
                  ...
                  it("Approve transfer ownership", function () {
                    return performNonFungibleTokenStatus(
                      nonFungibleTokenProperties.symbol,
                      token.NonFungibleToken.approveNonFungibleTokenOwnership
                    ).then((receipt) => {
                      expect(receipt).have.property("status").to.eq(1);
                      if (!silent)
                        console.log(
                  ...
                  it("Update NFT metadata - non owner", function () {
                    return issuerNonFungibleToken
                      .updateMetadata("Updated metadata")
                      .then((receipt) => {
                        expect(receipt).is.not.exist;
                      })
                      .catch((error) => {
                      ...
                      .then((receipt) => {
                        ...
                        return refresh(nonFungibleTokenProperties.symbol).then(
                          () => {
                            ...
                            expect(nonFungibleToken)
                              .have.property("state")
                              .have.property("metadata")
                              .is.eq("Updated metadata");
                            if (!silent)
                              console.log(
                                indent,
                                "Created Token:",
                                JSON.stringify(nonFungibleToken.state)
                              );
                          }
                        );
                      });
                      ...
                      .then(() => {
                        return performNonFungibleTokenStatus(
                          ...
                          token.NonFungibleToken
                            .approveNonFungibleTokenOwnership
                        ).then((receipt) => {
                          expect(receipt).have.property("status").to.eq(1);
                        });
                      })
                  ...
                  it("Freeze NFT", function () {
                    return performNonFungibleTokenStatus(
                      nonFungibleTokenProperties.symbol,
                      token.NonFungibleToken.freezeNonFungibleToken
                    ).then((receipt) => {
                      expect(receipt).have.property("status").to.eq(1);
                    });
                  });
                  ...
                  it("Unfreeze NFT", function () {
                    return performNonFungibleTokenStatus(
                      nonFungibleTokenProperties.symbol,
                      token.NonFungibleToken.unfreezeNonFungibleToken
                    ).then((receipt) => {
                      expect(receipt).have.property("status").to.eq(1);
                    });
                  });
                  ...
                  it("Mint item - challenge zero address", function () {
                    ...
                    return nonFungibleToken
                      ...
                      .catch((error) => {
                        expect(error)
                          .have.property("code")
                          .to.eq(errors.RECEIVER_KYC_REQUIRED);
                      });
                  });
                  ...
                  it("Update NFT item metadata - non holder", function () {
                    return NonFungibleTokenItem.fromSymbol(
                      ...
                      return nftItem
                        .updateMetadata("Trust me I am the item holder")
                        .then((receipt) => {
                          expect(receipt).is.not.exist;
                        })
                        .catch((error) => {
                          expect(error)
                            .have.property("code")
                            .to.eq(errors.UNEXPECTED_RESULT);
                        });
                    });
                  });
                  ...
                  it("Endorse", function () {
                    let nftItemInstance = new NonFungibleTokenItem(
                      symbol,
                      itemId,
                      wallet
                    );
                  ...
                  it("Endorse with metadata", function () {
                    ...
                    return nftItemInstance
                      .endorse("Hello blockchain")
                      .then((receipt) => {
                        expect(receipt).have.property("status").to.eq(1);
                      });
                  });
                  ...
                  it("Endorse with metadata and memo", function () {
                    ...
                    return nftItemInstance
                      .endorse("Hello Blockchain", { memo: "Hello Signer" })
                      .then((receipt) => {
                        expect(receipt).have.property("status").to.eq(1);
                      });
                  });
                  ...
                  it("Freeze item", function () {
                    return performNonFungibleTokenItemStatus(
                      symbol,
                      itemId,
                      token.NonFungibleToken.freezeNonFungibleTokenItem,
                      null
                    ).then((receipt) => {
                      expect(receipt).have.property("status").to.eq(1);
                      if (!silent)
                  ...
                  it("Unfreeze item", function () {
                    return performNonFungibleTokenItemStatus(
                      symbol,
                      itemId,
                      token.NonFungibleToken.unfreezeNonFungibleTokenItem,
                      null
                    ).then((receipt) => {
                      expect(receipt).have.property("status").to.eq(1);
                      if (!silent)
  ...
  function performNonFungibleTokenStatus(
    symbol: string,
    perform: any,
    overrides?: any
  ) {
    return perform(symbol, provider, overrides)
      .then((transaction) => {
        return token.NonFungibleToken.signNonFungibleTokenStatusTransaction(
          transaction,
          issuer
        );
      })
      .then((transaction) => {
        return token.NonFungibleToken.sendNonFungibleTokenStatusTransaction(
          transaction,
          middleware
        ).then((receipt) => {
          expect(receipt).have.property("status").to.eq(1);
  ...
  function performNonFungibleTokenItemStatus(
    symbol: string,
    itemID: string,
    perform: any,
    overrides?: any
  ) {
    return perform(symbol, itemID, provider, overrides)
      .then((transaction) => {
        return token.NonFungibleToken.signNonFungibleTokenItemStatusTransaction(
          transaction,
          issuer
        );
      })
      .then((transaction) => {
        return token.NonFungibleToken.sendNonFungibleTokenItemStatusTransaction(
          transaction,
          middleware
        ).then((receipt) => {
          expect(receipt).have.property("status").to.eq(1);
  ...
  function refresh(symbol: string) {
    return token.NonFungibleToken.fromSymbol(symbol, wallet, null)
      .then((token) => {
        expect(token).to.exist;
        nonFungibleToken = token;
        if (!silent)
          console.log(indent, "STATE:", JSON.stringify(nonFungibleToken.state));
      })
      .then(() => {
        return token.NonFungibleToken.fromSymbol(symbol, issuer, null).then(
          (token) => {
            expect(token).to.exist;
