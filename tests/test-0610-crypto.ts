...
import { manifold, utils } from "../src.ts/index";
...
describe("Suite: Crypto", function () {
  ...
  it("Compute shared secret", function () {
    let wallet1 = manifold.Wallet.fromMnemonic(nodeProvider.kyc.provider);
    expect(wallet1).to.exist;
    if (!silent) console.log(indent, "Wallet1:", JSON.stringify(wallet1));

    let wallet2 = manifold.Wallet.fromMnemonic(nodeProvider.kyc.issuer);
